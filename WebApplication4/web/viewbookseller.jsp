...
<%
    ...
    PreparedStatement ps = con.prepareStatement("SELECT * FROM trial_schema.users WHERE userid = ?");
    ...
    if(rs.next())
    {
        ...
        <tr> <td>NAME         : <td><%=rs.getString("uname") %> </tr>
        <tr> <td>ADDRESS      : <td><%=rs.getString("address") %></tr>
        <tr> <td>CONTACT INFO : <td><%=rs.getString("phone") %> </tr>
        <tr> <td>EMAIL         : <td><%=rs.getString("email") %> </tr>
        </table></center>   
        
        
    <%}%>
    <center><br><form action="querybooks.jsp" method="post"><input type="submit" value="Back to Search"></form></center>
